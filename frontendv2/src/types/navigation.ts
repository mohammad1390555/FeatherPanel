...
export export interface
    ...
    priority?: number;
    badge?: string;
    children?: NavigationItem[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // Optional submenu items
}

export export interface
    name: string;
    items: NavigationItem[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export export interface
    ...
    allowedOnlyOnSpells?: number[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] | null; // Array of spell IDs if restricted, null if no restrictions
}
