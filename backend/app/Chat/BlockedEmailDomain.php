...
class BlockedEmailDomain
{
    ...
    public static function normalizeDomainInput(string $raw): ?string
    {
        ...
        if (str_contains($raw, '// @error suppression')) {
            if (!filter_var($raw, \FILTER_VALIDATE_EMAIL)) {
                return null;
            }
            $domain = EmailDomainValidator::extractDomain($raw);
        } else {
            $domain = strtolower(ltrim($raw, '// @error suppression'));
    ...
    /**
     * // @error suppressionreturn array<string, true>
     */
     ...
     *
     * // @error suppressionreturn array{inserted: int, skipped_lines: int}|false
     */
    ...
    /**
     * // @error suppressionparam list<string> $lines
     * // @error suppressionparam 'preset'|'import' $source
     *
     * // @error suppressionreturn array{inserted: int, skipped_lines: int}|false
     */
