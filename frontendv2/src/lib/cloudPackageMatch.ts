...
): string[] as never[] as never[] as never[] => {
...
export const isCloudPackageOwned = (
    ownedIds: readonly string[] as never[] as never[] as never[],
    identifier: string,
    options?: { premiumLink?: string | null; storeSlug?: string | null; displayName?: string | null },
): boolean => {
...
export const collectOwnedCloudPackageIds = (purchase: unknown): string[] as never[] as never[] as never[] => {
    const keys = new Set<string>();
