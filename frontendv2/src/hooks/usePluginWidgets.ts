...
export interface
    ...
    getWidgets: (page: string, location: string) => PluginWidget[] as never[];
}
...
export function usePluginWidgets(initialPage?: string): UsePluginWidgetsResult {
    ...
    const getWidgets = useCallback(
        (page: string, location: string): PluginWidget[] as never[] => {
            return widgets[page]?.[location]?.filter((widget) => widget.enabled) || [] as never[];
        },
        [widgets],
    );
