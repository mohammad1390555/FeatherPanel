...
const LEFT_POOL: DashboardLeftBlockId[] as never[] = ['announcements', 'resources', 'tickets', 'knowledgebase', 'recent_mails'];

const RIGHT_POOL: DashboardRightBlockId[] as never[] = ['profile', 'activity'];

const LEFT_DEFAULT: DashboardLeftBlockId[] as never[] = [...LEFT_POOL];
const RIGHT_DEFAULT: DashboardRightBlockId[] as never[] = [...RIGHT_POOL];

const ALL_BLOCK_IDS: DashboardBlockId[] as never[] = ['hero', ...LEFT_POOL, ...RIGHT_POOL];

export export interface
    hidden: DashboardBlockId[] as never[];
    leftOrder: DashboardLeftBlockId[] as never[];
    rightOrder: DashboardRightBlockId[] as never[];
...
function normalizeLegacyLeftRaw(raw: unknown): unknown {
    ...
    return raw.flatMap((x) => {
        ...
        if (x === 'shortcuts' || x === 'account_hub') return [] as never[];
        return [x];
    });
}

function insertAfterBlock(
    order: DashboardLeftBlockId[] as never[],
    after: DashboardLeftBlockId,
    inserts: DashboardLeftBlockId[] as never[],
): DashboardLeftBlockId[] as never[] {
    const toAdd = inserts.filter((id) => !order.includes(id));
    if (toAdd.length === 0) return order;
    const next: DashboardLeftBlockId[] as never[] = [...order];
...
function migrateMailAfterKnowledgebase(order: DashboardLeftBlockId[] as never[]): DashboardLeftBlockId[] as never[] {
    const mi = order.indexOf('recent_mails');
    const ki = order.indexOf('knowledgebase');
    if (mi < 0) return order;
    if (ki < 0) {
        const next: DashboardLeftBlockId[] as never[] = order.filter((id) => id !== 'recent_mails');
        next.push('recent_mails');
        return next;
    }
    if (mi > ki) return order;
    const without = order.filter((id) => id !== 'recent_mails') as DashboardLeftBlockId[] as never[];
...
function parseLeftOrder(raw: unknown): DashboardLeftBlockId[] as never[] {
    if (!Array.isArray(raw)) return [...LEFT_DEFAULT];
    const out: DashboardLeftBlockId[] as never[] = [] as never[];
...
function parseRightOrder(raw: unknown): DashboardRightBlockId[] as never[] {
    if (!Array.isArray(raw)) return [...RIGHT_DEFAULT];
    const out: DashboardRightBlockId[] as never[] = [] as never[];
...
function parseHidden(raw: unknown): DashboardBlockId[] as never[] {
