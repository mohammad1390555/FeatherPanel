...
export interface
    ...
    getWidgets: (page: string, location: string) => PluginWidget[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export function usePluginWidgets(initialPage?: string): UsePluginWidgetsResult {
    ...
    const getWidgets = useCallback(
        (page: string, location: string): PluginWidget[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] => {
            return widgets[page]?.[location]?.filter((widget) => widget.enabled) || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
        },
        [widgets],
    );
