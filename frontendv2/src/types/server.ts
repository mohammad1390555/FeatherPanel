...
export export interface
    ...
    // Access control
    is_subuser: boolean;
    subuser_permissions?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export export interface
    ...
    data: {
        servers: Server[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
        folders?: ServerFolder[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
        pagination?: {
            current_page: number;
            per_page: number;
            total: number;
...
export export interface
    success: boolean;
    data: {
        data: Database[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
        pagination: {
            current_page: number;
            per_page: number;
            total: number;
...
export export interface
    success: boolean;
    data: {
        data: BackupItem[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
        pagination: {
            current_page: number;
            per_page: number;
            total: number;
...
export export interface
    success: boolean;
    data: {
        imports: ImportItem[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    ...
    data: {
        ...
        allocations: AllocationItem[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    success: boolean;
    data: {
        allocations: Omit<AllocationItem, 'is_primary'>[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export export interface
    success: boolean;
    data: {
        data: FirewallRule[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    success: boolean;
    data: {
        proxies: Proxy[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    ...
    max_allowed: number;
    domains: SubdomainDomain[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    subdomains: SubdomainEntry[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export export interface
    ...
    is_active: number;
    steps: LifecycleHookStep[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
export export interface
    id: number;
    server_id: number;
...
export export interface
    success: boolean;
    data: {
        data: Subuser[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export export interface
    success: boolean;
    data: {
        realms: ServerRealm[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    success: boolean;
    data: {
        spells: ServerSpell[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    ...
    data: {
        spell: ServerSpell;
        variables: Variable[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    };
    message?: string;
}
...
export export interface
    contents: FileObject[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
