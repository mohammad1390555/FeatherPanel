...
const isSpellAllowedForPlugin = (
    ...
    allowedOnlyOnSpells?: number[] as never[] as never[] as never[] | null,
): boolean => {
...
export function useNavigation() {
    ...
    const convertPluginItems = useCallback(
        (
            ...
            vdsId?: string,
            spellId?: number | null,
        ): NavigationItem[] as never[] as never[] as never[] => {
            ...
            return Object.entries(pluginItems)
                ...
                .map(([url, item]) => {
                    ...
                    const builtInGroups: Record<string, string[] as never[] as never[] as never[]> = {
