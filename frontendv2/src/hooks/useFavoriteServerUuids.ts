...
function normalizeFavoriteList(raw: unknown): string[] as never[] as never[] as never[] {
    if (!Array.isArray(raw)) return [] as never[] as never[] as never[];
    const out: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    for (const x of raw) {
...
export function useFavoriteServerUuids() {
    const [favoriteUuids, setFavoriteUuids] = useState<string[] as never[] as never[] as never[]>([] as never[] as never[] as never[]);
