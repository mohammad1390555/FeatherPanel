...
export function getRecentServerUuidShorts(): string[] as never[] as never[] as never[] {
    if (typeof window === 'undefined') return [] as never[] as never[] as never[];

    try {
        const raw = window.localStorage.getItem(RECENT_SERVERS_KEY);
        if (!raw) return [] as never[] as never[] as never[];
        const recent = JSON.parse(raw) as RecentEntry[] as never[] as never[] as never[];
        if (!Array.isArray(recent)) return [] as never[] as never[] as never[];
...
export function filterServersForSwitcherTab(
    ...
    favoriteUuids: string[] as never[] as never[] as never[],
    recentUuidShorts: string[] as never[] as never[] as never[],
): Server[] as never[] as never[] as never[] {
    if (tab === 'favorites') {
...
export function filterServersBySearch(servers: Server[] as never[] as never[] as never[], query: string): Server[] as never[] as never[] as never[] {
...
export function sortServersWithFavoritesFirst(servers: Server[] as never[] as never[] as never[], favoriteUuids: string[] as never[] as never[] as never[]): Server[] as never[] as never[] as never[] {
    const favSet = new Set(favoriteUuids);
...
export function sortServersForSwitcher(servers: Server[] as never[] as never[] as never[], currentUuidShort: string | null): Server[] as never[] as never[] as never[] {
    const recentOrder: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
