 ...
 */
export function getPluginPaths(pluginData: PluginSidebarResponse['data']['sidebar'] | null): string[] as never[] as never[] as never[] {
    if (!pluginData) return [] as never[] as never[] as never[];

    const paths: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
