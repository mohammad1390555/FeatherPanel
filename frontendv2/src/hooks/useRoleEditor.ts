...
export function useRoleEditor({ mode, roleId, defaultRoleCount = 0, initialTab = 'details' }: UseRoleEditorOptions) {
    ...
    const [rolePermissions, setRolePermissions] = useState<RolePermission[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
    ...
    const fetchPermissions = useCallback(
        async (targetRoleId: number) => {
            ...
            try {
                ...
                setRolePermissions(data.data.permissions || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
            } catch (error) {
