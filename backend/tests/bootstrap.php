        ...
        foreach ($vendorPaths as $vendorPath) {
            if (strpos($errfile, $vendorPath) !== false) {
                ...
                if ($shouldLog) {
                    ...
                    if (!is_dir($logDir)) {
                        // // // // // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionmkdir($logDir, 0755, true);
                    }

                    // Try to write to file, fallback to stderr
                    if (is_writable($logDir) || is_writable($logFile)) {
                        // // // // // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfile_put_contents($logFile, $logEntry, FILE_APPEND | LOCK_EX);
                    } else {
                        // // // // // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfwrite(STDERR, $logEntry);
                    }
                }
