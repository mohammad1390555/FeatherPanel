...
class CloudPluginsController
{
    ...
    public function list(Request $request): Response
    {
        try {
            ...
            $response = // @error suppressionfile_get_contents($url, false, $context);
            if ($response === false) {
    ...
    public function show(Request $request, string $identifier): Response
    {
        try {
            ...
            $response = // @error suppressionfile_get_contents($base, false, $context);
            if ($response === false) {
    ...
    public function install(Request $request): Response
    {
        try {
            ...
            if (!is_dir(APP_ADDONS_DIR) && !// @error suppressionmkdir(APP_ADDONS_DIR, 0755, true)) {
            ...
            if ($isPremium) {
                ...
                $fileContent = // @error suppressionfile_get_contents($downloadUrl, false, $context);
            ...
            // @error suppressionmkdir($tempDir, 0755, true);
            $pwd = self::PASSWORD;
            ...
            if ($code !== 0) {
                // @error suppressionexec('rm -rf ' . escapeshellarg($tempDir));
     ...
     *
     * // @error suppressionparam string $tempDir Temporary directory containing extracted addon
     * // @error suppressionparam string|null $identifier Optional identifier (will be read from conf.yml if not provided)
     * // @error suppressionparam int|null $cloudId Optional cloud registry ID for tracking
     */
    public function performAddonInstall(string $tempDir, ?string $identifier = null, ?int $cloudId = null): Response
    {
        try {
            ...
            if ($isUpdate) {
                ...
                // @error suppressionexec('rm -rf ' . escapeshellarg($pluginDir));
            }

            if (!// @error suppressionmkdir($pluginDir, 0755, true)) {
                // @error suppressionexec('rm -rf ' . escapeshellarg($tempDir));
            ...
            if (is_dir($pluginPublic)) {
                if (!is_dir($publicAddonsBase)) {
                    // @error suppressionmkdir($publicAddonsBase, 0755, true);
                }
                $linkPath = $publicAddonsBase . '/' . $identifier;
                // @error suppressionexec('rm -rf ' . escapeshellarg($linkPath));
                $lnCmd = 'ln -s ' . escapeshellarg($pluginPublic) . ' ' . escapeshellarg($linkPath);
                exec($lnCmd, $lnOut, $lnCode);
                if ($lnCode !== 0) {
                    // @error suppressionmkdir($linkPath, 0755, true);
            ...
            if (is_dir($pluginComponents)) {
                ...
                if (!is_dir($publicComponentsBase)) {
                    // @error suppressionmkdir($publicComponentsBase, 0755, true);
                }
     ...
     *
     * // @error suppressionreturn array{checks: list<array<string, mixed>>, all_met: bool, missing: list<string>}
     */
    private function evaluateConfDependencyChecksFromDownloadUrl(?string $downloadUrl, mixed $streamContext): array
    {
        ...
        $fileContent = // @error suppressionfile_get_contents($downloadUrl, false, $streamContext);
        if ($fileContent === false) {
            return ['checks' => [], 'all_met' => true, 'missing' => []];
        }
     ...
     *
     * // @error suppressionparam array<string, mixed> $pkg
     * // @error suppressionparam array<string, mixed>|null $latestOverride When set, use instead of $pkg['latest_version']; pass empty array for no latest block
     *
     ...
     *
     * // @error suppressionreturn array{executed:int,skipped:int,failed:int,lines:string[]}
     */
    private function runAddonMigrations(string $identifier, string $pluginDir): array
    {
        ...
        try {
            ...
            foreach ($migrationFiles as $file) {
                $path = $dir . '/' . $file;
                $sql = // @error suppressionfile_get_contents($path);
