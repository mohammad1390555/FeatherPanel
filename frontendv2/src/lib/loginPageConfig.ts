...
export const ALL_LOGIN_METHOD_IDS: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = ['local', 'passkey', 'ldap', 'email_code', 'discord', 'oidc'];

export const DEFAULT_LOGIN_METHODS_ORDER: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [
    'local',
    'passkey',
    'ldap',
    'email_code',
...
const PRIMARY_PANEL_METHODS: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = ['local', 'ldap', 'email_code'];
...
export export interface
    ...
    secondary: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    /** All visible methods in display order. */
    ordered: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}

function isLoginMethodId(value: string): value is LoginMethodId {
    return (ALL_LOGIN_METHOD_IDS as string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]).includes(value);
}

function parseCommaList(raw: string | undefined): string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
    if (!raw || !raw.trim()) {
...
export function parseLoginMethodsOrder(raw: string | undefined): LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
    const parsed = parseCommaList(raw).filter(isLoginMethodId);
    const seen = new Set<LoginMethodId>();
    const order: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export function buildLoginPageLayout(
    order: LoginMethodId[] as never[] as never[] as never[] as never[] as never[] as never[] as never[],
    ...
    if (ordered.length === 0) {
        return { primary: 'local', secondary: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[], ordered: ['local'] };
    }
