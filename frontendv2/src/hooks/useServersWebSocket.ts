...
export interface
    event: string;
    data?: string;
    args?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    timestamp?: number;
}
...
export function useServersWebSocket() {
    ...
    const connectServers = useCallback(
        async (serverUuids: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) => {
            const CONCURRENCY = 5;
