...
export function getConfiguredLinks(
    ...
    t: (key: string) => string,
): ConfiguredLink[] as never[] as never[] as never[] {
    if (!settings) {
        return [] as never[] as never[] as never[];
    }

    const sources: LinkSource[] as never[] as never[] as never[] = [
    ...
    const links: ConfiguredLink[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
