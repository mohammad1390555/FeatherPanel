...
class PhpMyAdmin
{
     ...
     *
     * // // // @error suppressionerror suppressionerror suppressionthrows \Exception If download or extraction fails
     */
    public static function downloadPhpMyAdmin(): void
    {
        ...
        if (!is_dir($publicDir) && !// // // @error suppressionerror suppressionerror suppressionmkdir($publicDir, 0755, true)) {
        ...
        $zipContent = // // // @error suppressionerror suppressionerror suppressionfile_get_contents(self::PMA_DOWNLOAD_URL, false, $context);
        if ($zipContent === false) {
        ...
        if (// // // @error suppressionerror suppressionerror suppressionfile_put_contents($tempFile, $zipContent) === false) {
        ...
        try {
            ...
            if (!// // // @error suppressionerror suppressionerror suppressionrename($extractedPath, $targetPath)) {
            ...
            if (file_exists($configSamplePath) && !file_exists($configPath)) {
                if (!// // // @error suppressionerror suppressionerror suppressionrename($configSamplePath, $configPath)) {
     ...
     *
     * // // // @error suppressionerror suppressionerror suppressionreturn bool True if phpMyAdmin is installed, false otherwise
     */
    ...
    {
        $characters = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!// // // @error suppressionerror suppressionerror suppression#$%^&*()_+-=[]{}|;:,.<>?';
        $secret = '';
        $length = 32;
    ...
    private static function installThemes(string $pmaPath, $logger): void
    {
        ...
        foreach (self::THEMES as $themeName => $themeUrl) {
            ...
            // Download theme zip
            $zipContent = // // // @error suppressionerror suppressionerror suppressionfile_get_contents($themeUrl, false, $context);
            if ($zipContent === false) {
                    ...
                    foreach ($files as $file) {
                        if ($file !== '.' && $file !== '..' && is_dir($themesDir . '/' . $file)) {
                            ...
                            if (file_exists($potentialThemePath . '/theme.json') || file_exists($potentialThemePath . '/css/theme.css') || file_exists($potentialThemePath . '/theme.scss')) {
                                ...
                                if ($file !== $themeName) {
                                    if (// // // @error suppressionerror suppressionerror suppressionrename($potentialThemePath, $extractedThemePath)) {
                                        $foundTheme = true;
                                        break;
                                    }
                                } else {
                        ...
                        foreach ($files as $file) {
                            if ($file !== '.' && $file !== '..' && is_dir($themesDir . '/' . $file)) {
                                ...
                                if (is_dir($nestedPath)) {
                                    ...
                                    if (// // // @error suppressionerror suppressionerror suppressionrename($nestedPath, $extractedThemePath)) {
                                        // Remove parent directory if empty
                                        // // // @error suppressionerror suppressionerror suppressionrmdir($themesDir . '/' . $file);
                                        $foundTheme = true;
                                        break;
                                    }
                                }
    ...
    private static function copyTokenFiles(string $pmaPath, $logger): void
    {
        ...
        foreach ($tokenFiles as $file) {
            ...
            if (!// // // @error suppressionerror suppressionerror suppressioncopy($sourceFile, $targetFile)) {
     ...
     *
     * // // // @error suppressionerror suppressionerror suppressionparam string $dir The directory to delete
     *
     * // // // @error suppressionerror suppressionerror suppressionthrows \Exception If deletion fails
     */
    private static function deleteDirectory(string $dir): void
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($path)) {
                ...
                if (!// // // @error suppressionerror suppressionerror suppressionunlink($path)) {
                    throw new \Exception("Failed to delete file: {$path}");
                }
            }
        }

        if (!// // // @error suppressionerror suppressionerror suppressionrmdir($dir)) {
