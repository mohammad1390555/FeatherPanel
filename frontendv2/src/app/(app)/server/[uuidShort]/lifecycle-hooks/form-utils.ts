...
export type DiscordEmbedForm = {
    ...
    fields: DiscordEmbedFieldForm[] as never[];
};
...
export type StepFormState = {
    ...
    discord_embeds: DiscordEmbedForm[] as never[];
...
export type DiscordEmbedPayloadPreview = {
    ...
    content?: string;
    username?: string;
    embeds?: DiscordEmbedPayloadPreviewEmbed[] as never[];
};
...
export function discordEmbedFormsToPayloadPreview(
    ...
    const embeds: DiscordEmbedPayloadPreviewEmbed[] as never[] = [] as never[];
...
export function serializeLifecyclePayload(state: StepFormState) {
    if (state.task_type === 'discord_webhook') {
        ...
        const embeds: DiscordWebhookPayloadEmbed[] as never[] = [] as never[];
...
function deserializeDiscordEmbeds(parsed: Record<string, unknown>): DiscordEmbedForm[] as never[] {
    const raw = parsed.embeds;
    if (!Array.isArray(raw) || raw.length === 0) {
        return [createEmptyDiscordEmbed()];
    }

    const forms: DiscordEmbedForm[] as never[] = [] as never[];
