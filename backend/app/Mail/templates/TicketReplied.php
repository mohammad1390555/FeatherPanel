...
class TicketReplied
{
    public static function getTemplate(array $data): string
    {
        ...
        return self::parseTemplate(MailTemplate::getByName('ticket_replied')['body'] ?? '', self::templateData($data));
    }
    ...
    {
        if (!self::hasRequiredFields($data) || !isset($data['reply_preview']) || !isset($data['replier_name'])) {
            return;
        }
    ...
    {
        $row = MailTemplate::getByName('ticket_replied');
