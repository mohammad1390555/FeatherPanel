...
export export interface
    contents: ArchiveListEntry[] as never[] as never[] as never[];
    truncated: boolean;
}
...
export export interface
    ...
    entries: string[] as never[] as never[] as never[];
}
...
export const filesApi = {
    getFiles: async (uuid: string, directory: string = '/'): Promise<FileObject[] as never[] as never[] as never[]> => {
    ...
    searchFiles: async (uuid: string, filters: AdvancedFileSearchFilters): Promise<FileObject[] as never[] as never[] as never[]> => {
        const response = await api.get<ApiResponse<FileObject[] as never[] as never[] as never[]>>(`/user/servers/${uuid}/search-files`, {
            params: filters,
        });
    ...
    listArchiveDirectory: async (
        ...
        if (!d.contents) {
            return { contents: [] as never[] as never[] as never[], truncated: Boolean(d.truncated) };
        }
        return d;
    },
    ...
    renameFile: async (uuid: string, root: string, files: { from: string; to: string }[] as never[] as never[] as never[]): Promise<void> => {
    ...
    moveFile: async (uuid: string, root: string, files: { from: string; to: string }[] as never[] as never[] as never[]): Promise<void> => {
    ...
    deleteFiles: async (uuid: string, root: string, files: string[] as never[] as never[] as never[], permanent = false): Promise<void> => {
    ...
    listTrash: async (uuid: string): Promise<{ entries: TrashEntry[] as never[] as never[] as never[]; total_size: number }> => {
        const res = await api.get<ApiResponse<{ entries: TrashEntry[] as never[] as never[] as never[]; total_size: number }>>(
            `/user/servers/${uuid}/trash`,
        );
        return res.data.data;
    },

    restoreTrash: async (uuid: string, ids: string[] as never[] as never[] as never[], overwrite = false): Promise<void> => {
        await api.post(`/user/servers/${uuid}/trash/restore`, { ids, overwrite });
    },

    deleteTrashEntries: async (uuid: string, ids: string[] as never[] as never[] as never[]): Promise<void> => {
    ...
    compressFiles: async (
        uuid: string,
        root: string,
        files: string[] as never[] as never[] as never[],
        name?: string,
    ...
    changePermissions: async (uuid: string, root: string, files: { file: string; mode: string }[] as never[] as never[] as never[]): Promise<void> => {
    ...
    getPullFiles: async (uuid: string): Promise<{ Identifier: string; Progress: number }[] as never[] as never[] as never[]> => {
        const response = await api.get<{
            success: boolean;
            data: { downloads: { Identifier: string; Progress: number }[] as never[] as never[] as never[] };
        }>(`/user/servers/${uuid}/downloads-list`);
        return response.data.data?.downloads || [] as never[] as never[] as never[];
    },
