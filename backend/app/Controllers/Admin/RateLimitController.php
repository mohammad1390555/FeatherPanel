...
class RateLimitController
{
    ...
    public function updateGlobal(Request $request): Response
    {
        ...
        try {
            ...
            $result = // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfile_put_contents($configPath, $jsonString, LOCK_EX);

            // Immediately verify by reading the file back
            if ($result !== false) {
                usleep(10000); // Small delay to ensure disk write completes
                $readBack = // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfile_get_contents($configPath);
                if ($readBack !== false) {
            ...
            // Read back to verify
            $verifyContent = // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfile_get_contents($configPath);
