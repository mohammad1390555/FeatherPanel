...
export function trashStatsFromList(data: { entries: TrashEntry[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; total_size: number }): TrashFolderStats {
...
export function filterFeatherTrashFiles<T extends { name: string }>(files: T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
    return files.filter((f) => !isFeatherTrashEntry(f.name));
}

export function filterFeatherTrashNames(names: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
    return names.filter((n) => !isFeatherTrashEntry(n));
}

export function filterSelectableFiles<T extends Pick<FileObject, 'isTrashShortcut'>>(files: T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
