...
export function useFileManager(serverUuid: string) {
    ...
    // State
    const [files, setFiles] = useState<FileObject[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [selectedFiles, setSelectedFiles] = useState<string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
    const [ignoredPatterns, setIgnoredPatterns] = useState<string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
    ...
    const refreshIgnored = useCallback(() => {
        ...
        if (saved) {
            ...
            setIgnoredPatterns([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
        }
    }, [serverUuid]);
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            ...
            setFiles(sorted);
            setSelectedFiles([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
        } catch (err) {
            console.error(err);
    ...
    const [activePulls, setActivePulls] = useState<{ Identifier: string; Progress: number }[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
...
function sortFiles(files: FileObject[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): FileObject[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
