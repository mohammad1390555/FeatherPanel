...
export function useUserServersList(enabled: boolean) {
    const [servers, setServers] = useState<Server[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
    ...
    const fetchServers = useCallback(async (force = false) => {
        ...
        try {
            ...
            const list = Array.isArray(response.servers) ? response.servers : [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
