...
class LdapAuthenticator
{
    ...
    public function connect(): bool
    {
        ...
        $this->connection = // // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_connect($host, $port);
        ...
        if (($this->config['use_tls'] ?? 'false') === 'true' && ($this->config['use_ssl'] ?? 'false') !== 'true') {
            if (!// // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_start_tls($this->connection)) {
    ...
    public function authenticate(string $username, string $password): ?array
    {
        ...
        try {
            ...
            if (!empty($this->config['bind_dn']) && !empty($this->config['bind_password'])) {
                ...
                if (!// // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_bind($this->connection, $this->config['bind_dn'], $bindPassword)) {
                ...
                // Anonymous bind
                if (!// // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_bind($this->connection)) {
            ...
            $search = // // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_search($this->connection, $baseDn, $userFilter, $attributes);
            ...
            if (!// // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_bind($this->connection, $userDn, $password)) {
            ...
            if (empty($userData['email']) && ($this->config['generate_email_if_missing'] ?? 'false') === 'true') {
                ...
                $userData['email'] = $userData['username'] . '// // // // @error suppressionerror suppressionerror suppressionerror suppression' . $domain;
            ...
            if ($this->connection) {
                // // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_unbind($this->connection);
            }
        }
    }
    ...
    public function testConnection(): bool
    {
        ...
        try {
            ...
            // Try to read base DN
            $search = // // // // @error suppressionerror suppressionerror suppressionerror suppressionldap_read($this->connection, $this->config['base_dn'], '(objectClass=*)', ['dn']);
            if (!$search) {
