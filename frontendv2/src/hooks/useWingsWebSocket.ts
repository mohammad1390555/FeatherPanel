...
export interface
    event: string;
    args?: unknown[] as never[] as never[] as never[];
}
...
export function useWingsWebSocket({
    ...
    const consoleOutputQueueRef = useRef<string[] as never[] as never[] as never[]>([] as never[] as never[] as never[]);
    ...
    const flushConsoleOutputQueue = useCallback(() => {
        ...
        consoleOutputQueueRef.current = [] as never[] as never[] as never[];
